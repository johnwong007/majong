require("app.Network.Http.HttpDefine")
...
local function onCallback(event,callback,tag)
	...
	if event.name ~= "progress" and event.name ~= "cancelled" then
		...
		--[[移除缓冲标记]]
	    -- if event.name ~= "progress" and NetCallBack.m_pLoadingView then 
...
--[[发送http请求]]
local function doSend(callback, tag, server, method, params, otherUrl, requestHeaderrs)
	...
	----------------
	local request = nil
	request = network.createHTTPRequest(function (event) onCallback(event,callback,tag) end,url,"POST")
	request.tag=tag
...
local function onDownloadCallback(event,callback,filePathAndName,progress)
...
local function doDownloadSend(callback, url, tag, filePathAndName, progress)
	local request = nil
	request = network.createHTTPRequest(function (event) onDownloadCallback(event,callback,filePathAndName,progress) end,url,"GET")
	request.tag=tag
...
function HttpClient:setSession(session)
	m_Session = session
end

function HttpClient:getLoginCtr(callback)
	doSend(callback, POST_COMMAND_GETLOGINCONTROL, "Account", "getLoginCtr") 
end

function HttpClient:uploadDeviceInfo(callback,info)
	local params = buildParams(info,DBChannel)
	doSend(callback, POST_COMMAND_UPLOADDEVICEINFO, "Analysis", "mobileDevicesToActivate", params) 
end
---
-- 融云服务器API - 获取token
--
...
--
function HttpClient:getRCToken(callback,userId,name,portraitUri)
	...
	doSend(callback, POST_COMMAND_GETRCTOKEN, "", "", params,
		 "https://api.cn.ronghub.com/user/getToken.json",self:createRCHTTPHeaders())
end
-- 创建融云 http headers
function HttpClient:createRCHTTPHeaders()
...
--
function HttpClient:ignoreRCMembers(callback,userId,chatroomId,minute)
	if device.platform == ios then
		...
		doSend(callback, POST_COMMAND_IGNORERCMEMBERS, "", "", params,
...
-- @return [json格式,在回调里处理]
--
function HttpClient:rollbackRCMembers(callback,userId,chatroomId)
	if device.platform == ios then
		local params = {['userId'] = URLEncoder:encodeURI(userId),['chatroomId'] = URLEncoder:encodeURI(chatroomId)}
		doSend(callback, POST_COMMAND_ROLLBACKRCMEMBERS, "", "", params,
			"https://api.cn.ronghub.com/chatroom/user/gag/rollback.json",self:createRCHTTPHeaders())
	end
end

function HttpClient:esunLogin(callback,name,password,type,ip,version)
...
function HttpClient:login(callback,name,password,version,isTestEnvironment,merge)
...
function HttpClient:debaoThreePlatFormLogin(callback,uid,timeStamp,ip,sign,loginType)
...
function HttpClient:DBHttpLogin(callback,username,password,ip,loginType,version,macAddr,nickname,sdkSign)
...
function HttpClient:debaoLoginForVersionControl(callback,username,password,ip,loginType,version,sign,macAddr,nickname,sdkSign)
...
function HttpClient:touristTurnDebao(callback,name,password,mac,sex,email)
...
function HttpClient:updatePassword(callback,oldPassword,newPassword)
...
function HttpClient:getServerId(callback,serverId)
	local params = buildParams(serverId)
	doSend(callback, POST_COMMAND_GETSERVERID, "Account", "getServerId", params) 
end

function HttpClient:getServerPort(callback,port,version)
...
function HttpClient:taskListAll(callback)
	doSend(callback, POST_COMMAND_taskListAll, "Activity", "taskListAll") 
end
--[[
	领取任务奖励
]]
function HttpClient:taskFinishAndReward(callback,activityId,jsonArgs)
	...
	if activityId == "206" then
		...
		doSend(callback, POST_COMMAND_taskFinishAndReward, "Activity", "taskFinishAndReward",params) 
	end
end
--[[ 
 * 获取公告(列表)
...
function HttpClient:getAnnounceList(callback)
...
function HttpClient:fetchLoginReward(callback)
...
function HttpClient:getAccountInfo(callback,isNetCallBack)
 ...
 * 附：
 * 	-403 => 未登录]]
function HttpClient:getUserTableListMobile(callback)
 ...
 * 附：
 * 	-403 => 未登录]]
function HttpClient:quickStart(callback)
	doSend(callback, POST_COMMAND_QUICKSTART, "Lobby", "quickStart")
end

function HttpClient:quickStartNew(callback)
	doSend(callback, POST_COMMAND_QUICKSTART_NEW, "Lobby", "quickStartNew")
end

--[[获取消息中心/消息数量]]
function HttpClient:GetAllNoticesInfo(callback,type)
...
--[[获取未读公告/消息的数量]]
function HttpClient:getNoReadNotice(callback)
	if TRUNK_VERSION == DEBAO_TRUNK then
...
function HttpClient:setApplePushToken(callback, userId, pushToken)
...
--[[读取未读取活动数量]]
function HttpClient:getNoReadActivity(callback)
	if TRUNK_VERSION == DEBAO_TRUNK then
...
function HttpClient:getIsNewYear(callback)
	doSend(callback, POST_COMMAND_ISNEWYEAR, "Activity", "isNewYearTimes")
end

--[[读取是否有锦标赛推广]]
function HttpClient:getMatchAdCtr(callback)
...
function HttpClient:buyIn(callback, tableId, chips)
...
function HttpClient:reBuy(callback, tableId, chips)
	local params = buildParams(tableId, chips)
...
function HttpClient:fetchRookieProtection(callback)
	doSend(callback, POST_COMMAND_FETCHROOKIEPROTECTION, "Admin", "fetchRookieProtection")
end

function HttpClient:getRookieProtectionConfig(callback)
...
function HttpClient:getQQUserInfo(callback, userId)
	local params = buildParams(userId)
...
function HttpClient:getUserProfitRanking(callback, userId)
	local params = buildParams(userId)
	doSend(callback, POST_COMMAND_GETUSERPROFITRANKING, "Analysis", "getUserProfitRanking", params)
end

function HttpClient:updateUserPortrait(callback, portrait)
...
function HttpClient:hudForMobile(callback, userId, version, from, refresh)
	if TRUNK_VERSION==DEBAO_TRUNK then
...
function HttpClient:addConcern(callback, dstUserId, dstUserName)
...
function HttpClient:applyFriend(callback, dstUserId, dstUserName)
	local params = buildParams(dstUserId, dstUserName)
...
function HttpClient:removeFriend(callback, dstUserId, dstUserName)
	local params = buildParams(dstUserId, dstUserName)
	doSend(callback, POST_COMMAND_REMOVEFRIEND, "Sns", "removeFriend", params)
end

function HttpClient:addFriend(callback, dstUserId, dstUserName, messageId)
	local params = buildParams(dstUserId, dstUserName, messageId)
	doSend(callback, POST_COMMAND_ADDFRIEND, "Sns", "addFriend", params)
end

function HttpClient:refuseFriend(callback, dstUserId, dstUserName, messageId)
	local params = buildParams(dstUserId, dstUserName, messageId)
	doSend(callback, POST_COMMAND_REFUSEFRIEND, "Sns", "refuseFriend", params)
end

function HttpClient:sendFriendMsg(callback, dstUserId, dstUserName, content)
...
function HttpClient:getFriendsNum(callback)
	doSend(callback, POST_COMMAND_GETFRIENDSNUMS, "Sns", "getFriendsNums")
end

--[[获取我的关注列表（包括牌桌、头像、等级等信息）（不包括已经是好友的玩家）]]
function HttpClient:getConcernListInfo(callback, offset, limit)
	local params = buildParams(offset, limit)
...
function HttpClient:getProfitRankList(callback, id_site, type, times, friends, p_num,offset)
...
function HttpClient:getChampionRankList(callback, limit, times, friends,offset)
	if TRUNK_VERSION==DEBAO_TRUNK then
...
function HttpClient:getleaderBoardByLevel(callback, offset, limit)
...
function HttpClient:getRankListInfo(callback, _type,offset,limit)
	--if TRUNK_VERSION==DEBAO_TRUNK then
		if _type=="LEVEL" then
			HttpClient:getleaderBoardByLevel(callback,offset or 0,limit or 20)
		elseif _type=="PROFIT" then
			HttpClient:getProfitRankList(callback,1,"win",0,0,offset or 0,limit or 20)
		elseif _type=="POINT" then
			HttpClient:getChampionRankList(callback,offset or 20,0,0,limit or 20)
		end
end

function HttpClient:getTotalBalanceBoard(callback, pay_type, limit, sort_type)
...
function HttpClient:getUserCompetitorListInfo(callback, offset, limit)
	local params = buildParams(offset, limit)
...
function HttpClient:FreshGuide(callback)
...
function HttpClient:getUserTableList(callback)
	doSend(callback, POST_COMMAND_GETUSERTABLELIST, "Lobby", "getUserTableList")
end

function HttpClient:joinActivity(callback, activityId, jsonArgs)
	local params = buildParams(activityId, jsonArgs)
	doSend(callback, POST_COMMAND_JoinActivity, "Activity", "joinActivity", params)
end

function HttpClient:finishFreshGuide(callback, userId)
	local params = buildParams(userId)
	doSend(callback, POST_COMMAND_finishFreshGuide, "Activity", "finishFreshGuide", params)
end

function HttpClient:getUserUseFuncInfo(callback, funcId)
	local params = buildParams(funcId)
	doSend(callback, POST_COMMAND_getUserUseFuncInfo, "Activity", "getUserUseFuncInfo", params)
end

function HttpClient:getActivityData(callback, activityId, jsonArgs)
	local params = buildParams(activityId, jsonArgs)
...
function HttpClient:getImmTableList(callback,tableType,level,tableStatus,sortBy,sortType,returnType,playType,tableOwner)
...
function HttpClient:getDiyTableList(callback,pay_type,level,tableType)
...
function HttpClient:getMacthList(callback,matchType,level,matchStatus,sortBy,sortType)
	if TRUNK_VERSION==DEBAO_TRUNK then
...
function HttpClient:getMatchUserList(callback, matchId)
...
function HttpClient:getPrizeInfo(callback, matchId, bonusName, usersNum)
...
function HttpClient:getGainInfoByName(callback, gainName)
	local params = buildParams(gainName)
	doSend(callback, POST_COMMAND_GETGAININFOBYNAME, "Admin", "getGainInfoByName", params)
end

function HttpClient:getMatchListByGroup(callback,matchType,level,matchStatus,sortBy,sortType,returnType)
...
function HttpClient:applyMatch(callback,matchId,ticket,withMsg)
	local params = nil
	if TRUNK_VERSION==DEBAO_TRUNK then
		local paramVec = {}
		paramVec[1] = matchId
...
function HttpClient:getActivityList(callback, macAddress)
	if TRUNK_VERSION==DEBAO_TRUNK then
...
function HttpClient:getActivityListNew(callback, args)
	local params = buildParams(args)
	doSend(callback, POST_COMMAND_NEWGETACTIVITYLIST, "Activity", "getActivityList", params)
end

function HttpClient:getActivityPrize(callback, activityid, macAddress)
	if TRUNK_VERSION==DEBAO_TRUNK then
...
-- function HttpClient:getItemList(callback)
-- 	doSend(callback, POST_COMMAND_GETITEMLIST, "Props", "getItemList")
-- end

function HttpClient:charge(callback, token, secret, item_id)
...
--[[--数据上报
--统计用户行为]]
function HttpClient:dataReport(callback, itid, data, expand)
	local params = buildParams(itid, data, expand)
	doSend(callback, POST_COMMAND_DATAREPORT, "Others", "dataReport", params)
end

--[[下载图片]]
function HttpClient:downloadFile(callback, url, fileName, needProgress, tag)
	tag = tag and tag or 0
	doDownloadSend(callback, url, tag, fileName)
end

--[[检查版本更新]]
function HttpClient:checkUpgrade(target, version, type)
	local content = ""
...
function HttpClient:getUserTimeWonInfo(target, bigPrecision, smallPrecision, siteId, limitId, userId)
...
function HttpClient:isFriend(callback, dstUserID)
	local params = buildParams(dstUserID)
	doSend(callback, POST_COMMAND_ISFRIEND, "Sns", "isFriend", params)
end

function HttpClient:getMatchInfo(callback, matchId, tableId)
	if TRUNK_VERSION==DEBAO_TRUNK then
...
function HttpClient:getItemList(callback, ptype, btype)
	local params = buildParams(ptype, btype)
	doSend(callback, POST_COMMAND_GETITEMLIST, "Props", "getItemList", params)
end

--[[获取玩家的任务列表]]
function HttpClient:getTaskList(callback)
	doSend(callback, POST_COMMAND_GETTASKLIST, "Task", "getTaskList")
end

--[[领取任务奖励]]
function HttpClient:getTaskPrize(callback, taskID)
	local params = buildParams(taskID)
	doSend(callback, POST_COMMAND_GETTASKPRIZE, "Task", "getTaskPrize", params)
end

--[[HappyHour和Task配置]]
function HttpClient:getTaskAndHappyHourConfig(callback)
	doSend(callback, POST_COMMAND_TASK_HAPPYHOUR_INFO, "Task", "getIconStatus")
end

--[[手机充值卡充值]]
function HttpClient:phoneCardCharge(callback,uid,cardType,cardValue,cardid,cardpwd,exchangetype,sign,phpid)
	local params = buildParams(uid,cardType,cardValue,cardid,cardpwd,sign,exchangetype)
	doSend(callback, POST_COMMAND_PHONECARD, "Account", "ztCharge", params)
end

--[[德堡充值卡充值]]
function HttpClient:debaoCardCharge(callback,uid,cardid,cardpwd,sign,phpid)
...
-- function HttpClient:getItemList(callback, ptype)
-- 	local params = buildParams(ptype)
-- 	doSend(callback, nil, "Props", "getItemList", params)
-- end

--[[下单]]
function HttpClient:createChargingOrder(callback,userid,exchangetype,ade_type,pay_channel,bank_type,totalFee,sign)
	local m_tag = nil
...
function HttpClient:getAccountInfoNew(callback)
	doSend(callback, POST_COMMAND_GETACCOUNTINFO, "Account", "getAccountInfo")
end

function HttpClient:ApplePaySuccessCallback(callback,userId,userName,receipt,innerOrderId,transactionIdentifier)	
	local content = ""
	content = content.."userid="..userId
	content = content.."&username="..userName
	content = content.."&orderId="..transactionIdentifier
	content = content.."&innerOrderId="..innerOrderId
	content = content.."&receipt="..receipt
	content = content.."&version="..URLEncoder:encodeURI(DBVersion)
	content = content.."&env="

	local tmpStr = userId .. userName .. transactionIdentifier .. innerOrderId .. receipt .. DBVersion
	local sign   = CMMD5Charge(tmpStr)
	content = content.."&sign="..sign
	local url = DOMAIN_URL.."/pay/appay.php?"
	doSend(callback, POST_COMMAND_APPPAYNOTIFYSERVER, "", "",content,url)
end

function HttpClient:reduceTencentGameCoin(callback, url,openid,access_token,pf,pfkey,pay_token,amt,props_name,zoneid,userId,userName,innerOrderId,asynCallBack)	
	...
	doSend(callback, POST_COMMAND_REDUCE_TENCENT_GAMECOIN, "", "","",content)
end

function HttpClient:queryTencentGameCoin(callback,url,openid,access_token,pf,pfkey, pay_token,  zoneid,  userId,  userName)	
	local content = url
	...
	doSend(callback, POST_COMMAND_QUERY_TENCENT_GAMECOIN, "", "","",content)
end

function HttpClient:jailbreakChargeSuccessRequestGoodsCallback(callback, url, userId, userName, orderId)	
...
--[[获取图像列表]]
function HttpClient:getPortraitPics(callback, fid)
...
function HttpClient:uploadFile(callback,uid,path)
	normal_info_log("HttpClient:uploadFile上传文件功能未实现")
end
...
function HttpClient:getChannelReportSwitch(callback)
	local version = currentVersion()
	local channel = string.sub(version, -5)
	local params = buildParams(channel)
...
function HttpClient:deleteSomePrivateMessages(callback, messageId)
...
function HttpClient:setShowNotify(callback, activiteId, bShow)
...
--[[获取首冲赠送比例]]
function HttpClient:getFirstPayRate(callback)
	doSend(callback, POST_COMMAND_FIRST_PAY_RATE, "Account", "getFirstPayRate")
end

--[[获取手机轮播推荐赛事]]
function HttpClient:getBroadCastMatchList(callback)
...
function HttpClient:getApplyMatch(callback)
...
function HttpClient:getKnapSack(callback)
...
function HttpClient:getUserTicketList(callback)
...
function HttpClient:quitMatch(callback, matchId)
	local params = buildParams(matchId)
	doSend(callback, POST_COMMAND_QUITMATCH, "Lobby", "quitMatch", params)
end

function HttpClient:quitDiyMatch(callback, matchId)
	local params = buildParams(matchId)
	doSend(callback, POST_COMMAND_QUIT_DIY_MATCH, "Lobby", "quitDiyMatch", params)
end

function HttpClient:weixinShare(callback, type)
	local params = buildParams(type)
	doSend(callback, POST_COMMAND_SHARETOWECHATREPORT, "Others", "weixinShare", params)
end

--[[svn]]
function HttpClient:sendBoardInfo(callback, hid, hname, hinfo, remark)
...
function HttpClient:getBoardInfo(callback, fid)
	local params = buildParams(fid)
	doSend(callback, POST_COMMAND_GETBOARDINFO, "Analysis", "getFavoriteHandsInfo", params)
end

function HttpClient:getESunCK(callback, username, password)
	normal_info_log("HttpClient:getESunCK未实现")
...
function HttpClient:sendVerifyMsg(callback, verifyMod, recver, username, platform)
	local params = buildParams(verifyMod, recver, username, platform)
	doSend(callback, POST_COMMAND_sendVerifyMsg, "Account", "sendVerifyMsg", params)
end

function HttpClient:verifyPhoneCode(callback, code)
	local params = buildParams(code)
	doSend(callback, POST_COMMAND_verifyPhoneCode, "Account", "verifyPhoneCode", params)
end

function HttpClient:getVerifyCode(callback, username)
	local params = buildParams(username)
	doSend(callback, POST_COMMAND_getVerifyCode, "Account", "getVerifyCode", params)
end

function HttpClient:resetPassword(callback, method, username, email, mobile)
	local params = buildParams(method, username, email, mobile)
	doSend(callback, POST_COMMAND_ResetPassword, "Account", "resetPassword", params)
end

function HttpClient:getLoginSwitch(callback)
	doSend(callback, POST_COMMAND_GET_LOGINSWITCH, "Account", "loginSwitch")
end

function HttpClient:getShopSwitch(callback)
	doSend(callback, POST_COMMAND_GET_SHOPSWITCH, "Props", "shopSwitch")
end

function HttpClient:requestUniqueUser(callback, wanname, username, password)
...
function HttpClient:registerPC(callback, name, password, sex, verifyCode)
	...
	doSend(callback, POST_COMMAND_REGISTERPC, "Account", "registerInnerMobile", params)
end

function HttpClient:quickRegister(callback, name, password, sex, ip, version, platform)
	local sign = "debao_fuck_ip"
...
function HttpClient:getRefreshReportSwitch(callback)
	local version = currentVersion()
	local channel = string.sub(version, -5)
	local params = buildParams(channel)
...
function HttpClient:getUserVipInfo(callback, userId)
	local params = buildParams(userId)
	doSend(callback, POST_COMMAND_getUserVipInfo, "User", "getUserVipInfo", params)
end
--[[
 * 修改玩家扩展信息（不需修改项则不用填写）
...
function HttpClient:updateUserExtensionInfo(callback, truename, sex, country, province, city, address_detail, idcard, wbai_name)
	local sign = "debao_fuck_ip"
...
function HttpClient:getAffiches(callback,userId)
	local regChannel = string.sub(DBVersion,-5,-1) --"DEBAO"
	...
	doSend(callback, POST_COMMAND_ANNOUNCEINFO, "Notice", "getAffiches",params)
end
--[[
	公告奖励领取
]]
function HttpClient:getAward(callback,userId,id)
	local params = buildParams(userId,id)
	doSend(callback, POST_COMMAND_ANNOUNCEAWARD, "Notice", "getAward",params)
end
--[[检查session]]
function HttpClient:checkGameSession(callback)
	local params = buildParams(myInfo.data.userName,myInfo.data.userId,myInfo.data.userSession)
	doSend(callback, POST_COMMAND_CHECKGAMESESSION, "User", "checkGameSession",params)
end
--[[检查session]]
function HttpClient:createGameSession(callback)
	local data = {}
	data.username = myInfo.data.userName
	data.userId   = myInfo.data.userId
	data.game_session = myInfo.data.phpSessionId
	data.client_version = DBVersion
	local args = json.encode(data)	

	local params = buildParams(args)
	doSend(callback, POST_COMMAND_CREATEGAMESESSION, "User", "createGameSession",params)
end

--[[创建私人房间]]
function HttpClient:useRoomCard(callback, pid, room_name, password, seat_num, blind_level, duration)
	password = crypto.md5(password)
	local params = buildParams(pid, room_name, password, seat_num, blind_level, duration)
	doSend(callback, POST_COMMAND_USE_ROOMCARD, "Props", "useRoomCard",params)
end

--[[创建单桌sng房间]]
function HttpClient:useMatchCard(callback,pid,room_name,password,seat_num,init_chips,up_seconds,
	...
	doSend(callback, POST_COMMAND_USE_MATCHCARD, "Props", "useMatchCard",params)
end

--[[私人局买入申请订单列表]]
function HttpClient:getBuyinApplyOrders(callback, table_id)
	local params = buildParams(table_id)
	doSend(callback, POST_COMMAND_BUYIN_APPLY_ORDERS, "Account", "getBuyinApplyOrders",params)
end
...
function HttpClient:getMyPriTable(callback, status)
	local params = buildParams(status)
	doSend(callback, POST_COMMAND_GET_MY_PRITABLE, "Account", "getMyPriTable",params)
end
--[[
	个人中心 朋友局纪录
]]
function HttpClient:getPriTableList(callback, uid, start, num)
	local params = buildParams(uid, start, num)
	doSend(callback, POST_COMMAND_GET_PRITABLE_LIST, "Account", "getPriTableList",params)
end
--[[
	个人中心 朋友局纪录排名
]]
function HttpClient:getPriTableUserList(callback, table_id, start, num)
	local params = buildParams(table_id, start, num)
	doSend(callback, POST_COMMAND_GET_PRITABLE_USER_LIST, "Account", "getPriTableUserList",params)
end

--[[根据configId获取tableId]]
function HttpClient:getDiyTableIdByFid(callback, id)
	local params = buildParams(id)
	doSend(callback, POST_COMMAND_GET_DiyTableIdByFid, "Lobby", "getDiyTableIdByFid",params)
end

--[[根据tableId获取configId]]
function HttpClient:getDiyFidByTableId(callback, id)
	local params = buildParams(id)
	doSend(callback, POST_COMMAND_GET_DiyFidByTableId, "Lobby", "getDiyFidByTableId",params)
end

--[[SNG报名]]
function HttpClient:applyDiyMatch(callback, tableId, password)
	password = crypto.md5(password)
...
function HttpClient:getUserExtentionInfo(callback, userId)
	local params = buildParams(userId)
	doSend(callback, POST_COMMAND_GET_getUserExtentionInfo, "User", "getUserExtentionInfo",params)
end
--[[
	* 创建俱乐部
...
function HttpClient:createClub(callback, club_name, club_type)
	local params = buildParams(club_name, club_type)
	doSend(callback, POST_COMMAND_GET_createClub, "Club", "createClub",params)
end
--[[
	获取俱乐部列表
]]
function HttpClient:getClubList(callback,start,num)
	local params = buildParams(start, num)
	doSend(callback, POST_COMMAND_GET_getClubList, "Club", "getClubList",params)
end
--[[
	获取俱乐部列表
	@param String $club_name 俱乐部名称
...
function HttpClient:searchClub(callback,club_name,owner_name)
	local params = buildParams(club_name, owner_name)
	doSend(callback, POST_COMMAND_GET_searchClub, "Club", "searchClub",params)
end
--[[
	查询已提交申请俱乐部列表
]]
function HttpClient:getClubApplyList(callback)
	doSend(callback, POST_COMMAND_GET_getClubApplyList, "Club", "getClubApplyList")
end
--[[
	查询俱乐部申请列表
	 * @param $club_id
...
function HttpClient:getReviewClubList(callback,club_id)
	local params = buildParams(club_id)
	doSend(callback, POST_COMMAND_GET_getReviewClubList, "Club", "getReviewClubList",params)
end
--[[
	* 申请加入俱乐部
	 * @param $club_id
...
function HttpClient:applyClub(callback,club_id)
	local params = buildParams(club_id)
	doSend(callback, POST_COMMAND_GET_applyClub, "Club", "applyClub",params)
end
--[[
	 * 批量审核玩家申请
	 * @param $club_id
...
function HttpClient:ReviewClubList(callback,club_id,apply_id,action)
	local params = buildParams(club_id,apply_id,action)
	doSend(callback, POST_COMMAND_GET_ReviewClubList, "Club", "ReviewClubList",params)
end
--[[
	 * 获取任务列表
	 * @return Array
]]
function HttpClient:getDailyTaskList(callback)
	doSend(callback, POST_COMMAND_GET_getDailyTaskList, "Club", "getDailyTaskList")
end
--[[
	 * 获取领奖资格
	 * @param $activity_id
...
function HttpClient:queryReward(callback,queryReward)
	local params = buildParams(queryReward)
	doSend(callback, POST_COMMAND_GET_queryReward, "Club", "queryReward",params)
end
--[[
	* 领取奖励
	 * @param $reward_id
...
function HttpClient:receiveRewards(callback,reward_id)
	local params = buildParams(reward_id)
	doSend(callback, POST_COMMAND_GET_receiveRewards, "Club", "receiveRewards",params)
end
--[[
	* 获取俱乐部信息
	 * @return Array
]]
function HttpClient:getClubInfo(callback,club_id)
	local params = buildParams(club_id)
	doSend(callback, POST_COMMAND_GET_getClubInfo, "Club", "getClubInfo",params)
end
--[[
	* 获取俱乐部成员身份
	 * @return Array
]]
function HttpClient:getMemberInfo(callback,club_id)
	local params = buildParams(club_id)
	doSend(callback, POST_COMMAND_GET_getMemberInfo, "Club", "getMemberInfo",params)
end
--[[
	* 获取俱乐部成员
	 * @param Int $club_id
...
function HttpClient:getClubMembers(callback,club_id,uid,start,num)
	local params = buildParams(club_id,uid,start,num)
	doSend(callback, POST_COMMAND_GET_getClubMembers, "Club", "getClubMembers",params)
end
--[[
	* 获取俱乐部动态
	 * @param Int $club_id
...
function HttpClient:getClubHistory(callback,club_id,num)
	local params = buildParams(club_id,num)
	doSend(callback, POST_COMMAND_GET_getClubHistory, "Club", "getClubHistory",params)
end
--[[
	 * 获取俱乐部公告
	 * @param Int $club_id
...
function HttpClient:getClubNotice(callback,club_id)
	local params = buildParams(club_id)
	doSend(callback, POST_COMMAND_GET_getClubNotice, "Club", "getClubNotice",params)
end
--[[
	 * 设置俱乐部公告
	* @param int $club_id
...
function HttpClient:saveClubNotice(callback,club_id,notice)
	local params = buildParams(club_id,notice)
	doSend(callback, POST_COMMAND_GET_saveClubNotice, "Club", "saveClubNotice",params)
end
--[[
	 * 给会员派发奖励
...
function HttpClient:sentMoneyToMember(callback,accept_uid,value,club_id,pay_type)
	local params = buildParams(accept_uid,value,club_id,pay_type)
	doSend(callback, POST_COMMAND_GET_sentMoneyToMember, "Club", "sentMoneyToMember",params)
end
--[[
	* 踢出俱乐部玩家
...
function HttpClient:kickOutMember(callback,accept_uid,club_id)
	local params = buildParams(accept_uid,club_id)
	doSend(callback, POST_COMMAND_GET_kickOutMember, "Club", "kickOutMember",params)
end
--[[
	* 任命俱乐部成员
...
function HttpClient:appointMember(callback,accept_uid,club_position,club_id)
	local params = buildParams(accept_uid,club_position,club_id)
	doSend(callback, POST_COMMAND_GET_appointMember, "Club", "appointMember",params)
end
--[[
	* 解散俱乐部
	 * @return Int
]]
function HttpClient:dissolveClub(callback,club_id)
	local params = buildParams(club_id)
	doSend(callback, POST_COMMAND_GET_dissolveClub, "Club", "dissolveClub",params)
end
--[[
	* 退出俱乐部
	 * @return Int
]]
function HttpClient:quitClub(callback,club_id)
	local params = buildParams(club_id)
	doSend(callback, POST_COMMAND_GET_quitClub, "Club", "quitClub",params)
end
--[[
	* 获取战队排行榜
	 * @return Array
]]
function HttpClient:getClubBoardList(callback,num)
	local params = buildParams(num)
	doSend(callback, POST_COMMAND_GET_getClubBoardList, "Club", "getClubBoardList",params)
end
--[[
	* 战队钻石兑换接口
	 * @param $club_id 战队ID
...
function HttpClient:useClubDiamondExchange(callback,club_id,goods_id)
	local params = buildParams(club_id,goods_id)
	doSend(callback, POST_COMMAND_GET_useClubDiamondExchange, "Club", "useClubDiamondExchange",params)
end
--[[
	* 战队成员捐赠钻石接口
	* @param $num 捐赠数量 
...
function HttpClient:donateDiamondToClub(callback,num)
	local params = buildParams(num)
	doSend(callback, POST_COMMAND_GET_donateDiamondToClub, "Club", "donateDiamondToClub",params)
end
--[[
	允许基金捐赠操作的战队白名单
]]
function HttpClient:donateFundWhiteList(callback)
	doSend(callback, POST_COMMAND_GET_donateFundWhiteList, "Club", "donateFundWhiteList")
end
--[[
	* 捐赠战队基金
     * @param $num 	数量
...
function HttpClient:donateFund(callback,num)
	local params = buildParams(num)
	doSend(callback, POST_COMMAND_GET_donateFund, "Club", "donateFund",params)
end
--[[
	* 邀请玩家加入自己所在战队
...
function HttpClient:inviteMember(callback,accept_uid)
	local params = buildParams(accept_uid)
	doSend(callback, POST_COMMAND_GET_inviteMember, "Club", "inviteMember",params)
end
--[[
	* 邀请玩家加入自己所在战队
	 * @param $accept_uid 被邀请人的UID
...
function HttpClient:acceptInvite(callback,club_id,oper_uid)
	local params = buildParams(club_id,oper_uid)
	doSend(callback, POST_COMMAND_GET_acceptInvite, "Club", "acceptInvite",params)
end

function HttpClient:priTableList(callback,club_id)
	local params = buildParams(club_id)
	doSend(callback, POST_COMMAND_GET_priTableList, "Club", "priTableList",params)
end
-------------------------------------战队接口结束------------------------------------------------

function HttpClient:checkVersion(callback)
	...
	doSend(callback, POST_COMMAND_GET_checkVersion, "","",nil,configIp)
end
...
function HttpClient:exchangePoint(callback,point_num)
	local params = buildParams(point_num)
	doSend(callback, POST_COMMAND_GET_exchangePoint, "Account", "exchangePoint",params)
end
--[[
	/**
	* 购买商品，作为礼物送给指定玩家
...
function HttpClient:buyAsGift(callback,ware_id,accept_uid,buy_num)
	local params = buildParams(ware_id,accept_uid,buy_num)
	doSend(callback, POST_COMMAND_GET_buyAsGift, "Props", "buyAsGift",params)
end
function HttpClient:getPerson(callback)
	doSend(callback, POST_COMMAND_GET_getPerson, "User", "getPerson")
end
--[[
  	个人信息：修改个人信息操作
]]
function HttpClient:updatePerson(callback,truename,idcard)
	local params = buildParams(truename,idcard)
	doSend(callback, POST_COMMAND_GET_updatePerson, "User", "updatePerson",params)
end

function HttpClient:tableLevelToGameAddr(callback,big_blind,small_blind)
...
function HttpClient:checkPlatformId(callback,username,loginType)
	local params = buildParams(username,loginType)
	doSend(callback, 0, "User", "checkPlatformId", params)
end

function HttpClient:checkIsEmailExist(callback,useremail)
	local params = buildParams(useremail)
	doSend(callback, 0, "User", "checkIsEmailExist", params)
end

function HttpClient:queryEarning(callback,userId)
	local params = buildParams(userId)
	doSend(callback, 0, "User", "queryEarning", params)
end

function HttpClient:getTimecardUsingLog(callback,userId,start_time,end_time)
	local params = buildParams(userId,start_time,end_time)
	doSend(callback, 0, "User", "getTimecardUsingLog", params)
end
return HttpClient
