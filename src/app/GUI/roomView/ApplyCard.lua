...
local ApplyCard = class("ApplyCard", function()
...
function ApplyCard:ctor(params)
	self.params = params or {}
	self.loc = self.params.loc or cc.p(0,0)
	self.animTime = self.params.animTime or 1
	self.bgPic = self.params.bgPic or "picdata/ApplyCard/check_communitycards.png"

	self:setNodeEventEnabled(true) 	

	self.m_applyDelayTime = 0
	self.smallBlind = tostring(self.params.smallBlind)
	self.price = price_table[self.smallBlind] or 50
end
function ApplyCard:create()
	self:initUI()
end
function ApplyCard:initUI()
	...
	self.m_clockBg = CMButton.new({normal = self.bgPic,pressed = self.bgPic}, handler(self, self.menuCallBack), {scale9 = false})  
	self.m_clockBg:setPosition(self.loc.x,self.loc.y+30)
	self:addChild(self.m_clockBg)
	...
	self.m_infoNode = display.newNode()
		:align(display.CENTER, self.m_clockBg:getPositionX(), self.m_clockBg:getPositionY()+50)
		:addTo(self, 1)

	self.m_infoBg = cc.ui.UIImage.new("picdata/ApplyCard/bg_price_tips.png")
	...
	self.m_infoLabel = cc.ui.UILabel.new({
		...
		:align(display.CENTER, 0, 5)
		:addTo(self.m_infoNode, 1)
end

function ApplyCard:setPokerBgPos(data)
	self.m_pokerBgPos = data
end

function ApplyCard:updateApplyCardButtonPos(publicCardNum)
	local posx = 600
...
function ApplyCard:menuCallBack()
...
function ApplyCard:doAnimation()
...
function ApplyCard:setViewVisible(isVisible)
	if isVisible then
...
function ApplyCard:updateApplyDelayTime(times)
	times = times or 0
...
function ApplyCard:getActualPrice()
	return self.price
end

function ApplyCard:updatePrice()
	self.m_applyButton:setButtonLabelString("normal", ""..self.price)
	self.m_hintLabel:setString(""..self.price)
end

function ApplyCard:addApplyDelayTimes()
	self.m_applyDelayTime = self.m_applyDelayTime+1
	self:getActualPrice()
	self:updatePrice()
end

function ApplyCard:showDelayView(remainTime)
	self:setVisible(true)
