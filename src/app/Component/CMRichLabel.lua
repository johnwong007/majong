...
local CMRichLabel = class("CMRichLabel", function()
...
CMRichLabel.__index      = CMRichLabel
CMRichLabel._fontName = nil
CMRichLabel._fontSize = nil
CMRichLabel._fontColor = nil
CMRichLabel._containLayer = nil --装载layer
CMRichLabel._spriteArray = nil --精灵数组
CMRichLabel._textStr = nil
CMRichLabel._maxWidth = nil
CMRichLabel._maxHeight = nil

--播放状态 1 表示未开始 2 表示播放中 3 表示已经播放完毕 
CMRichLabel._labelStatus = 1
...
function CMRichLabel:create(params)
	local ret = CMRichLabel.new()
	ret:init_(params)
	return ret
end

--设置text
function CMRichLabel:setLabelString(text)
...
--设置尺寸
function CMRichLabel:setDimensions(dimensions)
...
--获得label尺寸
function CMRichLabel:getLabelSize()
...
--是否在播放动画
function CMRichLabel:isRunningAmim()
...
--强制停止播放动画
function CMRichLabel:playEnded()
	print("play ended")
	self:removeNodeEventListenersByEvent(cc.NODE_ENTER_FRAME_EVENT)
...
--播放fade in 动画
function CMRichLabel:playFadeInAnim(wordPerSec)
...
function CMRichLabel:init_(params)
...
--获得每个精灵的尺寸
function CMRichLabel:getSizeOfSprites_(spriteArray)
	local widthArr = {} --宽度数组
...
--获得每个精灵的位置
function CMRichLabel:getPointOfSprite_(widthArr, heightArr, dimensions)
...
--调整位置（设置文字和尺寸都会触发此方法）
function CMRichLabel:adjustPosition_()
...
--创建精灵
function CMRichLabel:createSprite_(parseArray)
	local spriteArray = {}
...
--将字符串转换成一个个字符
function CMRichLabel:formatString_(parseArray)
	for i,dic in ipairs(parseArray) do
		local text = dic.text
		if text then
...
function CMRichLabel:parseString_(str)
...
--[[解析16进制颜色rgb值]]
function  CMRichLabel:convertColor_(xStr)
...
-- string.split()
function CMRichLabel:stringSplit_(str, flag)
	local tab = {}
	while true do
...
-- 拆分出单个字符
function CMRichLabel:stringToChar_(str)
    local list = {}
